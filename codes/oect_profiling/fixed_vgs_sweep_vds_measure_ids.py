...
max_number_samples_for_listv_keithley_source = 900
    # list of voltages
list_fvotl = np.arange(Vd_str, Vd_stop + volt_step, volt_step)
list_bvotl = np.arange(Vd_stop, Vd_str - volt_step, -volt_step)
...
print(f"{'='*5}\nupdate the //.tsp//\n{'='*5}")
file_tsp_path = r"C:\Users\20245580\LabCode\Codes_For_Experiments\codes\oect_profiling\fixed_vgs_sweep_vds_measure_ids.tsp"
with open(file_tsp_path, 'r') as file:
    lines = file.readlines()

ln_idx = 13
lines[ln_idx-1]= 'start_vd = ' + str(Vd_str)\
            + '\n' 
...
ln_idx = 90
lines[ln_idx-1]= '\t'+'smu_drain.trigger.source.listv({' + str(list_fvotl.tolist()).replace("[","").replace("]","") + "})"\
            + '\n' 

ln_idx = 102
lines[ln_idx-1]= '\t'+'smu_drain.trigger.source.listv({' + str(list_bvotl.tolist()).replace("[","").replace("]","") + "})"\
            + '\n' 
