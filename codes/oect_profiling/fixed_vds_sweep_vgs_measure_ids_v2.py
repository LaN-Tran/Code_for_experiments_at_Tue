...
max_number_samples_for_listv_keithley_source = 900
    # list of voltages
list_fvotl = np.arange(vg_str, vg_stop + volt_step, volt_step)
list_bvotl = np.arange(vg_stop, vg_str - volt_step, -volt_step)
n_listv = max(len(list_fvotl), len(list_bvotl))
print(f"{len(list_fvotl)=}")
print(f"{len(list_bvotl)=}")
...
print(f"{'='*5}\nupdate the //.tsp//\n{'='*5}")
file_tsp_path = r"C:\Users\20245580\LabCode\Codes_For_Experiments\codes\oect_profiling\fixed_vds_sweep_vgs_measure_ids_v2.tsp"
...
ln_idx = 23
lines[ln_idx-1]= 'points = ' + str(len(list_fvotl))\
            + '\n' 
...
ln_idx = 181
lines[ln_idx-1]= '\t'+'smu_gate.trigger.source.listv({' + str(list_fvotl.tolist()).replace("[","").replace("]","") + "})"\
            + '\n' 

ln_idx = 194
lines[ln_idx-1]= '\t'+'smu_gate.trigger.source.listv({' + str(list_bvotl.tolist()).replace("[","").replace("]","") + "})"\
            + '\n' 
...
try:
    for vds_iter in vd_sweep:
        ...
        for i in range(0, na_samples):
                        ...
                        measured_vg = float(keithley_instrument.query(f"print(smub.nvbuffer1.sourcevalues[{i}+1])"))
                        with open(file_path, 'a') as file: 
