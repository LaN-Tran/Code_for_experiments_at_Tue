...
arduino_sw_cntrl = 4 # (HIGH = OFF/ LOW = ON)
...
arduino_board.digital[arduino_sw_cntrl].write(1)
time.sleep(1)

        # path to the measurement record
file_path = "C:/Users/20245580/LabCode/Codes_For_Experiments/exp_data/20251111/ecram.csv"
...
nexp = 10
n_pulse_type_1 = 5
amp_pulse_type_1 = -0.7 # (Vgs > 0, decrease gm. bcz source is always 0, and drain - source are symmertrical)
...
n_pulse_type_2 = 5
amp_pulse_type_2 =  0.7 # (Vgs < 0, increase gm.  bcz source is always 0, and drain - source are symmertrical)
    ...
    for idx_exp in range(0, nexp):
            ...
            for idx_pulse_type_1 in range (0, n_pulse_type_1):
                ...
                keithley_instrument.smub.source.levelv = amp_pulse_type_1
                    # close switch
                time.sleep(sw_settle_time)
                arduino_board.digital[arduino_sw_cntrl].write(0)
                    # start pulse
                start_time = time.time()
                current_time = time.time()
